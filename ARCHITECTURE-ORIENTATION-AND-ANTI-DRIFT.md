# RYZ3N Architecture Orientation & Anti-Drift Capsule

**Status:** Navigation / architecture-recovery aid — **NOT CANON, NOT A NEW ARCHITECTURE LAYER**  
**Purpose:** Restore the correct RYZ3N mental model quickly, prevent duplicate architecture and abstraction inflation, and identify when deeper source reading is actually required.  
**Use first when:** resuming architecture work, reviewing a proposed capability/layer/Brain, entering a new chat/model/session, or recovering after context loss.

> **This file compresses orientation. It does not replace the authoritative sources it points to.**

---

## 1. The 60-second RYZ3N orientation

Canonical ontology:

`Creator → RYZ3N → ARCs → Brains → Agents → Execution`

Do **not** silently insert PRIME, OMEGA, FACTORY, Hermes, a model router, a mission controller, a world model, or any other implementation mechanism as a new canonical tier.

Current prototype/support roles:

- **RYZ3N** — the ecosystem architecture/platform identity above ARCs.
- **PRIME** — current supervisor/director/orchestrator during the prototype era; not an ARC and not a new canonical tier.
- **OMEGA** — current primary ARC quality controller / reliability sentinel and ARC-population steward under PRIME; not an ARC and not a canonical tier.
- **FACTORY** — durable ARC creation/lifecycle/provenance register and quality-by-construction production mechanism; not a Brain, ARC or authority tier.
- **BRAIN STEWARD** — Brain-population/lifecycle/boundary stewardship inside ARCs; does not replace ARC reasoning.
- **Hermes** — operational intelligence/execution infrastructure serving RYZ3N; not RYZ3N itself.
- **Models / LLMs / RLM patterns / future world-model substrates** — replaceable cognition suppliers, not holders of constitutional truth.

Primary constitutional quality principle:
...
When sources disagree or a material architecture decision is being interpreted, use the existing constitutional load order rather than conversational memory.

Compressed authority order:

1. **`Javalin13/ryzen-continuity` canonical governance / original conceptual corpus**
2. **Founder-approved constitutional evolution in `ryzen-core`**
3. **approved architecture / productization / runtime standards**
4. **authoritative ARC/domain source**
5. **current runtime evidence / tests / receipts / actual observed state**
6. **conversation, recollection, prose or model assertion**

For operational truth, preserve the constitutional discipline:

- **Reality over doctrine** when determining what is actually running.
- **Implementation over aspiration** when determining what exists today.
- **Evidence over theory** when determining whether a material claim is proven.

A document may define what **should** exist while runtime evidence shows what **does** exist. Do not collapse those categories.
...
Classify the proposal as one of only five outcomes:

1. **REUSE** — the capability already exists; use the existing owner/mechanism.
2. **CROSS-LINK** — the concern spans existing mechanisms; connect them rather than creating another one.
3. **REFINE** — deepen an existing capability/standard because a specific failure remains insufficiently covered.
4. **IMPLEMENT** — architecture already exists; the gap is engineering, not another concept.
5. **NEW CAPABILITY** — only when a materially distinct failure mode remains uncovered after checking existing authority and mechanisms.

**Default preference: REUSE → CROSS-LINK → REFINE → IMPLEMENT. `NEW CAPABILITY` is the exceptional outcome.**

A new name is not evidence of a new architectural need.
...
- canonical / constitutional alignment;
