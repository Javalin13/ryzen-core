# ARC Commercial Pipeline — 2026-09-11

```yaml
---
type: arc-commercial-pipeline-current
status: founder-directed-current-reality
created: 2026-09-11
classification: commercial-validation + authorized-creation-queue
scope: NARC pilot + KMS7 commercial follow-up
runtime_implementation_authorized: NARC-next-after-Cargo
amendable: true-additively
---
```

## Purpose

Record the Founder-directed commercial and ARC-creation decisions made on 2026-09-11 without overstating runtime, registration, customer acceptance or revenue.

This file supplements, and does not replace:

- `ARC-COMMERCIALIZATION-LAUNCH-AND-OPERATING-PLAN.md`;
- `ARC-V1-V6-COMMERCIAL-MATURITY-TUTORIAL.md`;
- `OMEGA-ARC-FACTORY-STEWARDSHIP-STANDARD.md`;
- PRIME `FACTORY.md` and OMEGA registry truth.

## NARC — next authorized ARC after Cargo

Founder decision:

- Display name: **NARC**.
- Intended owner/customer: **Narek**, fitness coach.
- Sequence: **next ARC after Cargo ARC**.
- Intended first use: help Narek reconstruct/revamp his website and support the business with the ARC operating model.
- `arc_id`: `narc` is reserved as the intended stable identifier, subject to normal birth-gate validation at creation.
- Current state on 2026-09-11: **Founder-authorized / pre-creation**.
- NARC is **not yet** to be reported as OMEGA-registered, FACTORY-active, runtime-provisioned, V1-verified or live until those evidence gates actually close.

### NARC founder pilot commercial terms

Founder offered Narek the same exceptional introductory structure used for the VONDA founding pilot:

- first **6 months free**;
- thereafter **€50/month** or **€500/year** under Standard scope/fair-use rules;
- Narek indicated he is willing to pay even before the free period ends;
- that willingness does not automatically alter the offered six-month-free arrangement unless the Founder and Narek explicitly change it.

This is a **Founder-authorized pilot exception**, not a new replacement for canonical Standard pricing or the Founding-20 framework.

ARC billing tier and ARC maturity remain separate. Narek is not purchasing a maturity color/version. NARC must still earn V1→V6 maturity through evidence.

## KMS7 Cars / Adnan — lower-cost ARC/Telegram proposal

Current commercial reality:

- prospect: **KMS7 Cars / Adnan**;
- earlier bespoke proposal referenced by the Founder: approximately **€17,000**;
- no acceptance was received from the latest prior outreach;
- on 2026-09-11 the Founder sent Adnan a voice message proposing a substantially cheaper ARC-based alternative;
- intended customer interface: **Telegram robot/system**;
- intended outcome: cover the necessary business-support/automation scope through the ARC model instead of a large bespoke build;
- working recurring price communicated: approximately **€150–€170/month**;
- current status: **outreach sent / awaiting reply**;
- this is **not** a closed deal, accepted subscription, invoice, booked revenue or authorized ARC creation yet.

Do not encode the phrase “100 times cheaper” as a literal accounting equivalence between the old one-time bespoke proposal and the new recurring subscription. It is the Founder’s qualitative sales framing; the commercial structures differ.

If Adnan accepts in principle, the next step is to convert the discussion into a defined ARC scope, commercial tier/exception, access/integration boundaries, privacy/data responsibilities and then an explicit ARC creation authorization before OMEGA/FACTORY birth registration.

## Commercial interpretation

These two developments support the current ARC productization thesis:

1. A real entrepreneur (Narek) accepted the value proposition strongly enough to volunteer early payment despite a free pilot period.
2. The ARC model provides a path to reframe previously expensive bespoke automation projects into managed recurring services.
3. Customer-facing delivery should remain outcome-led: website/interface reconstruction, automation, assistant/bot behavior and business execution — while the underlying PRIME/OMEGA/FACTORY mechanics remain internal.

This does **not** yet prove retention, scalable onboarding, KMS7 conversion, NARC runtime success or cohort economics. Those remain evidence questions.

## Execution order

1. Finish Cargo ARC’s current bounded provisioning/evidence work truthfully.
2. Instantiate NARC next through the Golden Blueprint + mandatory OMEGA/FACTORY birth contract.
3. Keep NARC’s pilot pricing marked as an exception, not global canonical pricing.
4. Await KMS7 response; do not create a KMS7 ARC or count revenue before explicit acceptance/authorization.
5. If KMS7 responds positively, scope the Telegram-first ARC offer against Standard vs heavier integration requirements before final commercial commitment.
